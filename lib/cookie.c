                                ...
                                origin */
{
  ...
  for(next = lineptr; next; fields++) {
    ...
    case 4:
      {
        /* do a strict check before expiration parsing to make sure that the
         * parsing function won't read past the current field. alternately, a
         * copy of the current field could be passed to the function instead.
         */
        char *endp;
        const char *p;
