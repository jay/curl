...
CURLcode Curl_loadhostpairs(struct SessionHandle *data)
{
  ...
  for(hostp = data->change.resolve; hostp; hostp = hostp->next) {
    ...
    /* The DNS cache entry 'dns' which was pulled from the hostcache is in use
    if dns->inuse > 0. However in the special case of an entry that was created
    via CURLOPT_RESOLVE (ie a prior pass of this code) and is in the hostcache
    the dns->inuse is always one more; so for example dns->inuse will be 1 even
    when not actually in use. A result of that is user specified entries will
    not be removed by normal means. We need to know whether the entry is
    _actually_ in use (hence the check below) because if it is we can't modify
    its address list and will have to orphan (ie remove from hostcache) and
    replace it.
    */
    if(dns && (dns->inuse > (dns->is_user_specified ? 1 : 0))) {
      Curl_dns_entry *old;
      Curl_addrinfo *copied_addrlist;
      /* If we have 'addr' we are either adding or removing an address from
      the address list. In that case we need the existing address list for
      our replacement entry. However our soon to be orphaned entry (and by
      extension its address list) is in use so we can't take possession of it.
      Instead we make a deep copy of the list for our replacement entry.
      One exception though: We do not allow adding addr to an address list
      that was not user specified so in that case we do not copy.
      */
      if(addr && dns->addr && (dns->is_user_specified || removing))
        copied_addrlist = Curl_ai_deep_copy_list(dns->addr);
      else
        copied_addrlist = NULL;
      /* If it's a user specified entry release our hold on it so that the
      garbage cleanup can free the memory when it's no longer in use.
      */
      ...
      */
      ...
      Only if the old entry is user-specified or we'll be adding to the new
      entry do we consider the new entry to be user-specified.
      */
